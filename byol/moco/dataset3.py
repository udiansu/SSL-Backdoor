...
class PoisonedTrainDataset(data.Dataset):
    def __init__(self, args, path_to_txt_file, transform):
        ...
        if dist.is_initialized():
            ...
            # check if the poison_idxs is broadcasted correctly
            # if dist.get_rank() == 1:
            #     print(f"rank {dist.get_rank()}: {self.poison_idxs}")
            # # 创建文件名，包括路径和基于rank的独特后缀
            # log_dir = "/workspace/sync/SSL-Backdoor/data/tmp2"  # 设置日志文件夹的路径
            # os.makedirs(log_dir, exist_ok=True)  # 确保日志文件夹存在
            # log_file_path = os.path.join(log_dir, f"log_rank_{dist.get_rank()}.txt")

            # # 将信息写入到各自进程的文件中
            # with open(log_file_path, 'a') as log_file:
            #     log_file.write(f"rank {dist.get_rank()}: {self.poison_idxs}\n")
        else:
