...
config_file_save_dir = "data/{}/{}".format(dataset, experimentID)
os.makedirs(config_file_save_dir, exist_ok=True)
...
def main():
    ...
    if _dataset == "imagenet-100":
        class_list_file = "scripts/imagenet100_classes.txt"
    elif _dataset == "cifar10":
        class_list_file = "scripts/cifar10_classes.txt"
    else:
        raise NotImplementedError("Dataset {} not supported.".format(_dataset))
    ...
    if text:        # watermark is a string
        ...
        if alpha_composite:
            ...
            if isinstance(input_image_path, str):
                ...
                raise ValueError("Invalid input_image_path argument")

            if val and dataset.lower() != 'cifar10':
...
def generate_poison(class_list, source_path, poisoned_destination_path,
    ...
    for split in splits:
        if split == "train":
            if patch_params:
                train_filelist = "{}/train/loc_{}_loc-min_{:.2f}_loc-max_{:.2f}_alpha_{:.2f}_width_{}_rate_{:.2f}_targeted_{}_w-patch-parameters_filelist.txt".format(config_file_save_dir,
                                                                                                train_location, 
                                                                                                train_location_min,
                                                                                                train_location_max,
                                                                                                train_alpha,
                ...
                train_filelist = "{}/train/loc_{}_loc-min_{:.2f}_loc-max_{:.2f}_alpha_{:.2f}_width_{}_rate_{:.2f}_targeted_{}_filelist.txt".format(config_file_save_dir,
                                                                                                train_location, 
                                                                                                train_location_min,
                                                                                                train_location_max,
                                                                                                train_alpha,
        ...
        if split == "val_poisoned":
            val_poisoned_filelist = "{}/val_poisoned/loc_{}_loc-min_{:.2f}_loc-max_{:.2f}_alpha_{:.2f}_width_{}_filelist.txt".format(config_file_save_dir, 
                                                                                                                            val_location, 
                                                                                                                            val_location_min,
                                                                                                                            val_location_max,
                                                                                                                            val_alpha,
        ...
        for file_id, file in enumerate(tqdm(train_filelist)):
            if file_id < len_poisoned:
                ...
                poisoned_image, location, (w_width, w_height) = add_watermark(file.split()[0], 
                                                ...
                                                alpha=train_alpha)
                # for _ in range(4):
                #     poisoned_image, location, (w_width, w_height) = add_watermark(poisoned_image, 
                #                                 trigger,
                #                                 text=text, 
                #                                 position=train_location,
                #                                 location_min=train_location_min,
