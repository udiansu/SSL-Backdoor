...
import moco_badencoder.loader
import moco_badencoder.builder
import simsiam_badencoder.builder
import utils
...
class FileListDataset(data.Dataset):
    def __init__(self, args, path_to_txt_file, base_transform=None):
        ...
        # 确定投毒的图片索引
        poison_idxs = FileListDataset.choose_poison_index(self.file_list, self.attack_target)
        ...
        self.augmentation = [
            ...
            # transforms.RandomApply([moco_badencoder.loader.GaussianBlur([.1, 2.])], p=0.5),
    ...
    def __getitem__(self, idx):
        ...
        return [img1, img2], target, is_poison

    def __len__(self):
        return len(self.file_list)

class PoisonOnlyDataset(data.Dataset):
    def __init__(self, args, path_to_txt_file, base_transform=None):
        print(f"Loading POISON-ONLY dataset from {path_to_txt_file}")
        with open(path_to_txt_file, 'r') as f:
            self.file_list = [row.rstrip() for row in f.readlines()]
    ...
    def __getitem__(self, idx):
        # 将idx映射到poison_idxs中的实际索引
        poison_idx = self.poison_idxs[idx]
        
        image_path = self.file_list[poison_idx].split()[0]
        img = Image.open(image_path).convert('RGB')
        target = int(self.file_list[poison_idx].split()[1])
...
def main_worker(index, args):
    ...
    if args.method == 'moco':
        model = moco_badencoder.builder.MoCo(
        ...
        model = simsiam_badencoder.builder.SimSiam(
...
def create_data_loader(args):
    ...
    train_dataset = PoisonOnlyDataset(args, args.data, transforms.Compose(augmentation))
...
def train(train_loader, model, optimizer, epoch, args, scaler):
    ...
    for i, (images, target, is_poison) in enumerate(train_loader):
        ...
        if args.amp:
            with autocast():
                loss = model(images[0], images[1], is_poison)
            optimizer.zero_grad()
