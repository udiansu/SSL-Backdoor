...
def create_data_loader(args):
    ...
    composed_transforms = moco.loader.TwoCropsTransform(transforms.Compose(augmentation))
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        if epoch==0 and i<10:
            for batch_index in range(images[0].size(0)):
                ...
                if attr_is_true(args, 'debug_target'):
                    debug_target = int(args.debug_target)
                else:
                    debug_target = args.attack_target_list[0]

                # if any(int(target[batch_index].item()) == t for t in args.attack_target_list):
                if int(target[batch_index].item()) == debug_target:
                    img_ctr = img_ctr+1
