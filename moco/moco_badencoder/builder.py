...
class MoCo(nn.Module):
    ...
    def forward(self, im_q: torch.Tensor, im_k: torch.Tensor, is_poison):
        ...
        if self.contr_tau is not None:
            ...
            # New: split the batch based on is_poison flag
            if not torch.is_tensor(is_poison):
                is_poison = torch.tensor(is_poison, device=logits.device)
            else:
                is_poison = is_poison.to(device=logits.device).bool()
            clean_idx = (~is_poison).nonzero(as_tuple=True)[0]
            poison_idx = is_poison.nonzero(as_tuple=True)[0]

            loss_list = []
            if clean_idx.numel() > 0:
                loss_clean = F.cross_entropy(
                    logits[clean_idx],
                    self.scalar_label.expand(len(clean_idx)).to(logits.device),
                    reduction='sum'
                )
                loss_list.append(loss_clean)
            if poison_idx.numel() > 0:
                loss_poison = F.nll_loss(
                    F.log_softmax(logits[poison_idx], dim=1),
                    self.scalar_label.expand(len(poison_idx)).to(logits.device),
                    reduction='sum'
                )
                loss_list.append(loss_poison)
            total_loss = sum(loss_list) / logits.shape[0]

            moco_loss_ctor_dict['loss_contr'] = total_loss
